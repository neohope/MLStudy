...
class ConstNode(object):
    """
    Desc:
        ConstNode对象，为了实现一个输出恒为 1 的节点（计算偏置项 wb 时需要）
        常数项对象，即相当于计算的时候的偏置项
    """
    def __init__(self, layer_index, node_index):
        """
...
class Connection(object):
    """
    Desc:
        Connection 对象，记录连接权重和连接所关联的上下游节点
    """
...
class ConnectionOp(object):
    """
    Desc:
        ConnectionOp 对象，提供 Connection 集合的操作
    """
    def __init__(self):
        """
        Desc:
...
class Normalizer(object):
    ...
    def norm(self, number):
        ...
        Returns:
            规范化之后的数据
        """
        # 此方法就相当于判断一个 8 位的向量，哪一位上有数字，如果有就将这个数设置为  0.9 ，否则，设置为 0.1，
        # 通俗比较来说，就是我们这里用 0.9 表示 1，用 0.1 表示 0
        return list(map(lambda m: 0.9 if (number!=np.nan and m!=np.nan) else 0.1, self.mask))
